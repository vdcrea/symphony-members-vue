<?php
	...
	-------------------------------------------------------------------------*/
		...
		public function checkFrontendPagePermissions($context) {
			...
			// member_action['login'] and member_action['logout']/?member_action=logout
			// are the only two supported at this stage.
			if(isset($_REQUEST['member_action']) && is_array($_REQUEST['member_action'])){
				list($action) = array_keys($_REQUEST['member_action']);
			}
			else if(isset($_REQUEST['member_action'])) {
				$action = $_REQUEST['member_action'];
			}
