<?php
	...
	Class eventMembers_Reset_Password extends MembersEvent {
		...
		const ROOTELEMENT = 'members_reset_password';
		...
		public static function documentation(){
			...
			return '
				...
				&lt;form method="post"&gt;
					...
					&lt;label&gt;Recovery Code: &lt;input name="fields[password][recovery_code]" type="text" value="{$code}"/&gt;&lt;/label&gt;
		...
		protected function __trigger(){
			...
			if(
				!isset($fields[$this->driver->getMemberDriver()->section->getFieldHandle('authentication')]['recovery_code'])
				or empty($fields[$this->driver->getMemberDriver()->section->getFieldHandle('authentication')]['recovery_code'])
			) {
				$result->setAttribute('result', 'error');
				$result->appendChild(
			...
			$row = Symphony::Database()->fetchRow(0, sprintf("
					...
					WHERE reset = 'yes'
					AND `recovery-code` = '%s'
				", $auth->get('id'), Symphony::Database()->cleanValue($fields[$this->driver->getMemberDriver()->section->getFieldHandle('authentication')]['recovery_code'])
			));
